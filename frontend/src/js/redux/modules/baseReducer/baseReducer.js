...
export const createReducer = (storeId, endpoint, formName = undefined, resourceList = undefined) => {
    ...
    const crear = data => (dispatch) => {
        ...
        api.post(endpoint, data).then(() => {
            NotificationManager.success('Registro creado', 'Éxito', 1000);
            if (resourceList) dispatch(push(resourceList));
        }).catch((error) => {
            console.log(error);
            for (const err in error) {
                NotificationManager.error(error[err][0], 'ERROR', 1000);
            }
        }).finally(() => {
            dispatch(setLoader(false));
        });
    ...
    const editar = (id, data) => (dispatch) => {
        ...
        api.put(`${endpoint}/${id}`, data).then(() => {
            NotificationManager.success('Registro actualizado', 'Éxito', 1000);
            if (resourceList) dispatch(push(resourceList));
        }).catch(() => {
    ...
    const eliminar = id => (dispatch) => {
        ...
        api.eliminar(`${endpoint}/${id}`).then(() => {
            dispatch(listar());
            NotificationManager.success('Registro eliminado', 'Éxito', 1000);
        }).catch(() => {
            NotificationManager.success('Error en la transacción', 'Éxito', 1000);
        }).finally(() => {
            dispatch(setLoader(false));
        });
    };
